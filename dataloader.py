...
class CellImagesDataset(Dataset):
    ...
    def __init__(self, parent_folder, metadata_path = None):
        ...
        for root, dirs, files in os.walk(self.parent_folder):
            for file_name in files:
                if file_name in ('Background Subtracted Image.tif','Background.tif','flows.tif','masks.tif','.svg'):
                    continue
                else:
    ...
    def __getitem__(self, idx):
        ...
        if md is not None:
            ...
            raise ValueErorr('No metadata found.')
...
class LoadImageBatch(Dataset):
    ...
    def __init__(self, parent_folder, metadata_path = None):
        ...
        self.dataset = CellImagesDataset(parent_folder = parent_folder, metadata_path = metadata_path)
        ...
        for i, file_size in enumerate(self.dataset.get_file_sizes()):
            ...
            if total_size > .375 * self.total_free_memory:
